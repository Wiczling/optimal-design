...
Priors are based on the following article "Comparison of Chromatographic Stationary Phases Using a Bayesian-Based Multilevel Model" [@kamedulska_toward_2022,]. In this work a simplified approach is taken with the focus on the neutral, acidic and basic forms of analytes and a single chromatographic stationary phase (XBridge Shield RP18 column [@kubik_XBridgeC18_2022,]), assuming the pKa and changes of pKa with organic modifier are known.
...
The simulation involved n = 500 analytes with log P and analyte type (acid and base) drawn at random from the following distributions:  $logP_i \sim normal(2,2.2)$ and $Analyte_i \sim categorical(A=0.5,B=0.5)$. Log P values were constrained to a range between 0.5 and 7. For single pH scenarios the analyte form distribution was 40% for neutral, 30% for acidic, and 30% for basic form. 
