...
```{r paths}
#' folder shortcuts:
figures_dir1 <- here("deliv", "figures", "design2_all","N")
tables_dir1  <- here("deliv", "tables", "design2_all","N")
figures_dir2 <- here("deliv", "figures", "design2_all","NI")
tables_dir2  <- here("deliv", "tables", "design2_all","NI")
figures_dir3 <- here("deliv", "figures", "design2_all","MeOH")
tables_dir3  <- here("deliv", "tables", "design2_all","MeOH")
figures_dir4 <- here("deliv", "figures", "design2_all","ACN")
tables_dir4  <- here("deliv", "tables", "design2_all","ACN")

figures_dir5 <- here("deliv", "figures", "manuscript_optional")
tables_dir5  <- here("deliv", "tables", "manuscript_optional")
...
thisScript <- "optimal-design-for-mixtures-design2-all"
```
...
Priors are based on the follwoing article "Comparison of Chromatographic Stationary Phases Using a Bayesian-Based Multilevel Model" by Paweł Wiczling and Agnieszka Kamedulska published in Analytical Chemistry 2024 96 (3), 1310-1319. 

In this work a simplified approach is taken with the focus on the neutral, acidic and basic forms of an analytes and single chromatographic stationary phase - assuming the pKa and changes of pKa with organic modifier are known.
...
Here I consider a design space that consist of linear organic modifier gradients ranging from 10 to 90 min and organic modifier content ranging from 0.05 to 0.2 for MeOH and ACN.

```{r design-space}
hplcparam_candidates_df = expand_grid(tg=seq(10,90,by=10), 
                                          td=2.1, 
                                          to=0.532,
                                          te=0.04, 
...
load(file = "../data/results-mixtures-design2.RData")
```
...
Probability for neutral, acidic and basic is 0.4, 0.3, 0.3 with logP being sampled from 2.2+-2 and constrained between 0.5 and 7.
...
```{r single-form-analytes-optimization-tgconstrain, eval=FALSE}
#p=2 and 3 are the same as for unconstrained
chosen_indices = c(17,18,65,66) # initial values
fwstgc_prior_4 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan, datastruct_prior_ma, inits, chosen_indices, tg_constraint = 270,n_iter = 100)
chosen_indices = c(17,18,65,66) # initial values
fwstgc_noprior_4 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan, datastruct_noprior_ma, inits, chosen_indices, tg_constraint = 270,n_iter = 100)

chosen_indices = c(1,39,24,65,68) # initial values
fwstgc_prior_5 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan, datastruct_prior_ma, inits, chosen_indices, tg_constraint = 270,n_iter = 100)
chosen_indices = c(1,39,24,65,68) # initial values
fwstgc_noprior_5 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan, datastruct_noprior_ma, inits, chosen_indices, tg_constraint = 270,n_iter = 100)
```

### Read the results and compare
#### Unconstrained
```{r single-form-analytes-logDet-summary}
chosen_indices_prior = c(17,18,65,66) # ref
chosen_indices_noprior= c(17,18,65,66) # ref
...
chosen_indices_prior   = c(23,71,65,68) # p=4
chosen_indices_noprior = c(23,71,65,68) # p=4
...
chosen_indices_prior = c(66,23,71,65,70) # p=5
chosen_indices_noprior = c(66,23,71,65,72) # p=5
...
#### tg constrained
```{r single-form-table-tgc, eval=FALSE}
data_ma=hplcparam_candidates_df%>%filter(xpH==0)%>%
  mutate(line=1:n())

combined_results = list(
prior_ref = fwstgc_prior_ref$chosen_indices,
noprior_ref = fwstgc_noprior_ref$chosen_indices,
prior_2 = fwstgc_prior_2$chosen_indices,
noprior_2 = 10000+fwstgc_prior_2$chosen_indices, # dummy
prior_3 = fwstgc_prior_3$chosen_indices,
noprior_3 = fwstgc_noprior_3$chosen_indices,
prior_4 = fwstgc_prior_4$chosen_indices,
noprior_4 = fwstgc_noprior_4$chosen_indices,
prior_5 = fwstgc_prior_5$chosen_indices,
noprior_5 = fwstgc_noprior_5$chosen_indices)
...
my.ggsave(.plot=plot1, .filename=paste("single-form-analytes-table-tgc.pdf"), .path = tables_dir1)
my.ggsave.png(.plot=plot1, .filename=paste("single-form-analytes-table-tgc.png"), .path = tables_dir1)
...
# iiresults<-iresults %>%
#     pivot_wider(names_from = "method", values_from = ivar) %>%
#   mutate(d_prior_2 = prior_2/prior_ref,
#          d_prior_3 = prior_3/prior_ref,
#          d_noprior_3 = noprior_3/noprior_ref,
#          d_prior_4 = prior_4/prior_ref,
#          d_noprior_4 = noprior_4/noprior_ref,
#          d_prior_5 = prior_5/prior_ref,
#          d_noprior_5 = noprior_5/noprior_ref)%>%
#   pivot_longer(d_prior_2:d_noprior_5, names_to = "method")%>%
#   select(-c(prior_ref:noprior_5))

# test2=iresults %>%
#   ggplot(aes(x=logP,y=ivar, color=as.factor(Type)))+
#   geom_point()+
#   labs(x="logP", y="max(Var)", color = "Type")+   
#   scale_y_continuous(
#                      trans='log10',
#                       breaks=scales::trans_breaks('log10', function(x) 10^x)(c(0.001, 1000)),
#                       labels=scales::trans_format('log10', scales::math_format(10^.x))) + 
...
# data_ma_pH%>%mutate(line = 1:n())%>%filter(tg %in% c(30,90), fio==0.05)%>%select(line)%>%pull()
chosen_indices = c(33,34,35,36,129,130,131,132)
# quick test:
...
chosen_indices = c(1,46,135,141,142,129,130,143,136)
...
chosen_indices = c(1,139,46,135,141,142,129,130,143,136)
...
chosen_indices = c(33,34,35,36,129,130,131,132) #init
fwtgc_prior_8 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan_pH, datastruct_prior_ma_pH, inits_pH, chosen_indices, tg_constraint = 480, n_iter = 100)
chosen_indices = c(33,34,35,36,129,130,131,132) #init
fwtgc_noprior_8 = fedorov_wynn_stan_par_tgconstraint(mean_log_det_stan_pH, datastruct_noprior_ma_pH, inits_pH, chosen_indices, tg_constraint = 480,n_iter = 100)
```

### Read the results and compare
```{r monoprotic-analytes-logDet-summary}

chosen_indices_prior   = c(130,139,137) # p=3
fw_prior_3 = fedorov_wynn_stan_par(mean_log_det_stan_pH, datastruct_prior_ma_pH, inits_pH, chosen_indices_prior, n_iter = 0)
chosen_indices_prior   = c(137,130,139,136) # p=4
fw_prior_4 = fedorov_wynn_stan_par(mean_log_det_stan_pH, datastruct_prior_ma_pH, inits_pH, chosen_indices_prior, n_iter = 0)
chosen_indices_prior   = c(142,137,130,139,136) # p=5
fw_prior_5 = fedorov_wynn_stan_par(mean_log_det_stan_pH, datastruct_prior_ma_pH, inits_pH, chosen_indices_prior, n_iter = 0)
chosen_indices_prior = c(33,34,35,36,129,130,131,132) # ref 
chosen_indices_noprior= c(33,34,35,36,129,130,131,132) # ref 
...
chosen_indices_prior   = c(141,142,129,130,139,136) # p=6
chosen_indices_noprior = c(45,46,133,130,139,136) # p=6
...
chosen_indices_prior   = c(45,141,142,129,130,139,136) # p=7 
chosen_indices_noprior = c(142,45,46,133,130,139,136) # p=7
...
chosen_indices_prior   = c(46,135,141,142,129,130,143,136) # p=8 
chosen_indices_noprior = c(46,139,141,142,129,130, 45, 136) # p=8
...
chosen_indices_prior   = c(132, 46, 135, 141, 142, 129, 130, 143, 140) # p=9 
chosen_indices_noprior   = c(45, 46, 135, 141, 142, 129, 130, 143, 136) # p=9 
...
chosen_indices = c(132,45,46,135,141,142,129,130,143,140) # p=10 
fw_prior_10 = fedorov_wynn_stan_par(mean_log_det_stan_pH, datastruct_prior_ma_pH, inits_pH, chosen_indices, n_iter = 0)
chosen_indices = c(45,132,46,135,141,142,129,130,143,144) # p=10 
fw_noprior_10 = fedorov_wynn_stan_par(mean_log_det_stan_pH, datastruct_noprior_ma_pH, inits_pH, chosen_indices, n_iter = 0)

 chosen_indices_prior   = c(33,30,47,46,141,130,135,136) # p=8 
 chosen_indices_noprior = c(29,46,45,30,133,130,139,132) # p=8
...
chosen_indices = c(33) # initial values
...
chosen_indices = c(36,33) # initial values
...
chosen_indices = c(9,33,36) # initial values
...
chosen_indices = c(1,9,33,36) # initial values
...
chosen_indices_prior = c(9,33) # ref
chosen_indices_noprior= c(9,33) # ref
...
chosen_indices_prior   = c(33,36) # p=2
chosen_indices_noprior = c(12,33) # p=2
...
chosen_indices_prior   = c(12,33,36) # p=3
chosen_indices_noprior = c(12,33,36) # p=3
...
chosen_indices_prior = c(12,34,33,36) # p=4
chosen_indices_noprior = c(12,11,33,36) # p=4
...
prior_1 = 10000+fws_MeOH_prior_1$chosen_indices,
...
chosen_indices = c(16,33) # initial values
...
chosen_indices = c(12,34,33,16) # initial values
...
chosen_indices_prior   = c(16,33) # p=2
chosen_indices_noprior = c(12,33) # p=2
...
chosen_indices_prior = c(12,36, 33, 11) # p=4
chosen_indices_noprior = c(12, 34, 33, 36) # p=4
...
prior_1 = 10000+fws_ACN_prior_1$chosen_indices,
...
```{r save-temp, eval=F}
save.image("results-mixtures-d2-temp.RData")
```
