...
array[nAnalytes] vector[6] paramsim; // logkw, S1m, dS1
 
 for (i in 1 : nAnalytes) {
  A1B2[i] = categorical_rng([0.5,0.5]');
  logPobs[i] = normal_lub_rng(2.2,2,0.5,7);
  ...
  paramsim[i] = multi_normal_rng(miu[i], Omega[A1B2[i]]);
 }
 
 {
 for (z in 1 : nSel){
   for (i in 1 : nAnalytes){
    real logkw_sim = paramsim[i,1]	+ hplcparam[z,8]*paramsim[i,4];								
    real S1_sim = paramsim[i,2] + hplcparam[z,8]*(paramsim[i,5]+hplcparam[z,7]*paramsim[i,6])+hplcparam[z,7]*paramsim[i,3];
    real S2_sim = 10^(-0.31+hplcparam[z,7]*0.42);
   y_hat_sim[i,z] = chromgratrapz(steps[selected[z]], logkw_sim, S1_sim, S2_sim, hplcparam[selected[z]]);
