...
transformed parameters {
 ...
 for (z in 1 : nObs){
   real logkw = param[1];
   real S1 = param[2] + param[3]*hplcparam[z,7];
   real S2 = 10^(-0.31 + 0.42*hplcparam[z,7]);
...
vector[11] izo_MeOH_sim;
vector[11] izo_ACN_sim;

 {
 for (z in 1 : nObs_sim){
   real logkw_sim = param[1];
   real S1_sim = param[2]+param[3]*hplcparam_sim[z,7];
   real S2_sim = 10^(-0.31+0.42*hplcparam_sim[z,7]);
  ...
  for (z in 1 : 11){
   ...
   real S1_sim = param[2] ;
   real S2_sim = 10^(-0.31);
   izo_MeOH_sim[z] = hplcparam_sim[1,4] + hplcparam_sim[1,3] *(1+exp(funlnki(logkw_sim, S1_sim, S2_sim, fi_sim[z])));
  }
 for (z in 1 : 11){
   real logkw_sim = param[1];
   real S1_sim = param[2] + param[3]*1;
   real S2_sim = 10^(-0.31 + 0.42*1);
   izo_ACN_sim[z] = hplcparam_sim[1,4] + hplcparam_sim[1,3] *(1+exp(funlnki(logkw_sim, S1_sim, S2_sim, fi_sim[z])));
  }
}
}
